...
function App() {
  const search = () => {
    ...
    const screenshotContainer = document.querySelector("img#screenshot") as HTMLImageElement;
    ...
    chrome.storage.local.get("screenshotURLs", (res) => {
      ...
      screenshotContainer.src = screenshotURL || chrome.runtime.getURL("images/answer_not_found.png");
    });
  }

  const clear = () => {
    if (confirm("Are you sure you want to delete all your answer screenshots?")) {
      chrome.storage.local.set({ screenshotURLs: {}, homeworkInfos: {} });
    }
  }

  const changeBookworkCodeSelect = (event: React.ChangeEvent<HTMLSelectElement>) => {
    chrome.storage.local.get("screenshotURLs", (res) => {
      const bookworkCodes = res.screenshotURLs[event.target.value];

      const bookworkCodeInput = document.querySelector("select#bookwork-code-input") as HTMLSelectElement;
  ...
  const updateAutoNext = () => {
    const autoNext = (document.querySelector("input#auto-next") as HTMLInputElement).checked;

    chrome.storage.local.set({ autoNext });
  }

  const openURL = (event: React.MouseEvent<HTMLButtonElement>) => {
    chrome.runtime.sendMessage({ action: "openURL", url: (event.target as HTMLButtonElement).value });
  }

  // Everytime the popup is opened, the date & type select and auto next checkbox are updated.
  useEffect(() => {
    chrome.storage.local.get(["screenshotURLs", "homeworkInfos", "autoNext"], (res) => {
      const homeworkInfos: {[key: string]: string} = {}

      if (res.screenshotURLs instanceof Object) {
        for (const packageId in (res.screenshotURLs as {[key: string]: {[key: string]: string}})) {
          homeworkInfos[packageId] = "Unnamed";
        }
      }
  
      if (res.homeworkInfos instanceof Object) {
        Object.entries((res.homeworkInfos as {[key: string]: string})).forEach(([packageId, homeworkInfo]) => {
      ...
      const packageIdInput = document.querySelector("select#package-id-input") as HTMLSelectElement;
      ...
      const autoNextInput = document.querySelector("input#auto-next") as HTMLInputElement;
      ...
      <div className="input-container">
        ...
        <div>
          <label htmlFor="auto-next" className="tooltip">
            Auto-next
            <span className="tooltip-text">Automatically clicks 'next' buttons (i.e. Continue, Next task).</span>
          </label>
          <input type="checkbox" id="auto-next" onChange={updateAutoNext}></input>

          <button onClick={openURL} value="https://www.sparxmaths.com/student/">Go to Sparx International</button>
          <button onClick={openURL} value="https://www.sparxmaths.uk/student/">Go to Sparx UK</button>
        </div>
      </div>
