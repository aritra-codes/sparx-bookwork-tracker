...
const screenshotAnswer = (packageID: string, bookworkCode: string) => {
    chrome.tabs.captureVisibleTab((screenshotURL) => {
        chrome.storage.local.get("screenshotURLs", (res) => {
            const screenshotURLs: {[key: string]: {[key: string]: string}} = checkInstance(res.screenshotURLs, Object);
...
chrome.runtime.onMessage.addListener(request => {
    if (request.action === "takeScreenshot") {
        ...
        chrome.storage.local.get("homeworkInfos", (res) => {
            let homeworkInfos: {[key: string]: string} = checkInstance(res.homeworkInfos, Object);

            homeworkInfos[request.packageId as string] = request.homeworkInfo as string;
